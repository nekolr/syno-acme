...
HTTP_PROXY_URL="127.0.0.1:10809"
...
installAcme () {
  ...
  ACME_SH_NEW_VERSION=$(wget -e http_proxy=${HTTP_PROXY_URL} -qO- -t1 -T2 "https://api.github.com/repos/acmesh-official/acme.sh/releases/latest" | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g')
